...
from scripts.argument import ArgumentProcessor
    ...
    async def process_case(self, session, case_href, case_id):
        try:
            ...
            if not case_details or not self.validate_case_details(case_details):
                ...
                oral_argument_audio = case_details.get("oral_argument_audio", [])
            ...
            # Argument Processing
            argument_dir = os.path.join(case_dir, self.settings.ARGUMENT_DIR)
            self.file_utils.create_directory(argument_dir)
            await self.argument_processor.process_argument(session, oral_argument_audio, argument_dir)
